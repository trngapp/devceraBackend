...
router.use(cors({ origin:['https://gentle-mushroom-02a86d610.1.azurestaticapps.net','https://www.devcera.com'],credentials:true}));
//router.use(cors());
...
router.post("/signup", async (req,res)=>{
    ...
    router.post("/login", async (req,res)=>{
       ...
       const user = await User.findOne({$and:[{email:emai}]});
            ...
            res.cookie('access_token',token,{sameSite:'none',path:'/',httpOnly:true}).send(`you are logged in!!-> ${token}`);
